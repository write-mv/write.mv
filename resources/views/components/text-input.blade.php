...
<div class="{{$attributes->get('class')}}">
    <label for="{{$attributes->whereStartsWith('wire:model')->first()}}"
           class="block text-sm leading-5 font-medium text-gray-700 fontfam-regular mb-1">{{$label}}
    </label>
    <div class="mt-1 relative rounded-md shadow-sm">
        <input
            ...
            @else
            class="block w-full shadow-sm focus:border-blue-500 sm:text-sm border-none rounded-md"
            @endif
