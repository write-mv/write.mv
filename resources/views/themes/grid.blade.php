<section class="container px-4 py-24 mx-auto">
    <h2 class="mb-2 text-3xl font-extrabold leading-tight text-gray-900" style="font-family: Poppins;">
        {{$blog->site_title}}</h2>
    <p class="mb-20 text-lg text-gray-500" style="font-family: Poppins;">{{$blog->description}}</p>
    <div class="grid grid-cols-1 gap-12 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">
        @foreach ($posts as $post)
        <div>
            @if($post->featured_image)
            <a href="#">
                <img src="{{$post->featured_image}}" class="object-cover w-full h-56 mb-5 bg-center rounded" alt="Kutty"
                    loading="lazy" />
            </a>
            @endif
            @if($post->is_english)
